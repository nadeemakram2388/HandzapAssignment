...
open class FormTextField: UITextField {
    ...
    fileprivate final func setup() {
        ...
        self.addTarget(self, action: #selector(_editingChanged), for: .editingChanged)
        self.addTarget(self, action: #selector(_editingEnd), for: .editingDidEnd)
    }
    
    @objc open func _editingChanged() {
        updateControl(true)
        updateTitleLabel(true)
        didChange?(self.text)
    }
    
    @objc open func _editingEnd() {
        didEndEditing?(self.text)
    }
    ...
    open var placeholderColor: UIColor = AppColor.color(.lineColor) {
        didSet {
    ...
    open var titleColor: UIColor = AppColor.color(.lineColor) {
        didSet {
    ...
    open var lineColor: UIColor = AppColor.color(.lineColor) {
        didSet {
            self.updateLineView()
        }
    }
    
    open var selectedLineColor: UIColor = AppColor.color(.textInputColor) {
        didSet {
            self.updateLineView()
        }
    }
    
    open var errorColor: UIColor = AppColor.color(.errorColor) {
        didSet {
            self.updateColors()
        }
    }
    
//    open var hintColor: UIColor = UIColor.lightGray {
//        didSet {
//            self.updateHintColor()
//        }
//    }
    ...
    open var hint: String? {
        didSet {
            self.updateControl(true)
            //self.updateErrorLabel(withHint: true)
        }
    }
    ...
    fileprivate func createLineView() {
        ...
        self.lineHeight = 1.0 * onePixel // self.lineHeight uses self.lineView so have to set it first
        self.selectedLineHeight = 1.5 * self.lineHeight
    ...
    fileprivate func createErrorLabel() {
        ...
        label.textColor = self.hasError ? self.errorColor : AppColor.color(.hintColor)
    ...
    fileprivate func updateErrorLabel() { //withHint: Bool = false
        self.errorLabel.text = error ?? hint
        self.errorLabel.textColor = error != nil ? AppColor.color(.errorColor) : AppColor.color(.hintColor)
        self.invalidateIntrinsicContentSize()
    }
    ...
    override open func textRect(forBounds bounds: CGRect) -> CGRect {
        ...
        let leftViewWidth = self.leftView?.frame.size.width ?? 0
        let rect = CGRect(x: 0 + leftViewWidth , y: titleHeight, width: bounds.size.width - leftViewWidth, height: bounds.size.height - titleHeight - lineHeight - errorHeight)
        return rect
    }
    ...
    open func errorLabelRectForBounds(_ bounds: CGRect) -> CGRect {
        ...
        guard let text = (error ?? hint) else { return CGRect(origin: .zero, size: CGSize(width: 0, height: 18)) }
        ...
        let boundingRect = text.boundingRect(with: s, options: .usesLineFragmentOrigin, attributes: textAttributes, context: nil)
        return CGRect(x: bounds.size.width - boundingRect.size.width, y: bounds.size.height - boundingRect.size.height, width: boundingRect.size.width, height: boundingRect.size.height)
    }
    ...
    open override var description: String {
        return "[FormTextField(\(String(describing: placeholder))) text:\(String(describing: text))]"
    }
